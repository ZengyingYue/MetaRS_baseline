...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1,
                 base_width=64, dilation=1, norm_layer=None,iw=0):
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False,
                 ...
                 norm_layer=None, deep_stem=False,wt_layer = [0,0,0,0,0,0,0]):
        super(ResNet, self).__init__()
        if norm_layer is None:
            norm_layer = nn.BatchNorm2d
        self._norm_layer = norm_layer
        self._wt_layer = wt_layer
        self.inplanes = 64
        self.dilation = 1
        ...
        self.layer1 = self._make_layer(block, 64, layers[0],wt = wt_layer[3])
        self.layer2 = self._make_layer(block, 128, layers[1], stride=2,
                                       dilate=replace_stride_with_dilation[0],wt = wt_layer[4])
        self.layer3 = self._make_layer(block, 256, layers[2], stride=2,
                                       dilate=replace_stride_with_dilation[1],wt = wt_layer[5])
        self.layer4 = self._make_layer(block, 512, layers[3], stride=2,
                                       dilate=replace_stride_with_dilation[2],wt = wt_layer[6])
    ...
    def _make_layer(self, block, planes, blocks, stride=1, dilate=False,wt=0):
        ...
        for i in range(1, blocks):
            layers.append(block(self.inplanes, planes, groups=self.groups,
                                ...
                                norm_layer=norm_layer,iw = 0 if i<blocks-1 else wt))
...
def resnet50_v1c(pretrained=False, progress=True, **kwargs) -> nn.Module:
    """Constructs a ResNet-50 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
        progress (bool): If True, displays a progress bar of the download to stderr
    """
    return _resnet('resnet50_v1c', Bottleneck, [3, 4, 6, 3], pretrained, progress, deep_stem=True,
                   **kwargs)


def resnet101_v1c(pretrained=False, progress=True, **kwargs):
    """Constructs a ResNet-101 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
        progress (bool): If True, displays a progress bar of the download to stderr
