...
def _make_conv_impl(in_channels, out_channels, kernel_size,
                    stride=1, dilation=1, use_bn=False, use_relu=False, init_fn=None):
    return ConvBlock(
        in_channels, out_channels, kernel_size, stride,
        padding=ConvBlock.same_padding(kernel_size, dilation),
        dilation=dilation, bias=False if use_bn else True, bn=use_bn, relu=use_relu,
        init_fn=init_fn
    )

def conv_with_kaiming_uniform(use_bn=False, use_relu=False):
    return partial(_make_conv_impl, use_bn=use_bn, use_relu=use_relu, init_fn=init_conv)
...
def select_channels(x, load_rgb = False, concat = False,in_channels = 3):
    if not concat:
        if load_rgb:
            x=x[:,-in_channels:,:,:]
        else:
        ...
        x = torch.concat([x[:,-in_channels:,:,:],x[:,:in_channels,:,:]],dim = 1)
    return x